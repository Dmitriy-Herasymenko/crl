...
// Fetches a single static WP page (not a post) by slug — e.g. "platni-poslugy".
// Returns null if no matching published page exists.
export async function fetchPageBySlug(slug, fetchInit = {}) {
  const res = await wpFetch(`${WP_BASE}/pages?slug=${encodeURIComponent(slug)}`, fetchInit);
  if (!res.ok) throw new Error(`WP API request failed: /pages?slug=${slug} -> ${res.status}`);
