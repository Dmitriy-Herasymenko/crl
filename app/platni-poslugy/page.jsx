...
export const metadata = {
  title: 'Платні послуги — Уманська центральна районна лікарня',
  description: 'Тарифи на платні медичні послуги КНП «Уманська центральна районна лікарня» Паланської територіальної громади.',
};
...
async function getDocuments() {
  const page = await fetchPageBySlug('platni-poslugy', { next: { revalidate: REVALIDATE_SECONDS } });
  if (!page) return [];
  return extractDocumentLinks(page.content.rendered);
}

export default async function PlatniPoslugyPage() {
  let documents = [];
  let loadError = false;
  ...
  try {
    ...
    console.error('Failed to load platni-poslugy from uman.crl.net.ua:', err);
    loadError = true;
  }
      ...
      <section className="page-header-section relative pt-40 pb-16 lg:pt-48 lg:pb-20 bg-gray-950 overflow-hidden">
        ...
        <div className="relative z-10 max-w-screen-xl mx-auto px-6 lg:px-10">
          <nav className="flex items-center gap-2 text-xs text-white/40 mb-6" aria-label="Хлібні крихти">
            ...
            <span>/</span>
            <span className="text-white/70">Платні послуги</span>
          </nav>
          ...
          <h1 className="text-4xl lg:text-5xl font-300 text-white leading-tight">
            Платні <span className="font-600">послуги</span>
          </h1>
      ...
      <section className="py-16 lg:py-20 bg-white">
        <div className="max-w-screen-xl mx-auto px-6 lg:px-10">
          <div className="reveal max-w-3xl mb-10">
            <p className="text-xs font-semibold tracking-widest text-blue-600 uppercase mb-4">Тарифи та документи</p>
            <h2 className="text-2xl sm:text-3xl font-600 text-gray-900 leading-snug">
              Тарифи на платні медичні послуги КНП «Уманська ЦРЛ»
            </h2>
          </div>
