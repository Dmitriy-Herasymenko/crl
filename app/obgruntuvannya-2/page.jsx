...
export const metadata = {
  title: 'Обґрунтування, відкриті торги — Уманська центральна районна лікарня',
  description: 'Обґрунтування технічних і якісних характеристик та очікуваної вартості предмета закупівлі КНП «Уманська центральна районна лікарня» (Постанова КМУ №710).',
};

async function getGroups() {
  const page = await fetchPageBySlug('obgruntuvannya-2', { next: { revalidate: REVALIDATE_SECONDS } });
  if (!page) return [];
  return extractYearGroupedDocuments(page.content.rendered);
}

export default async function ObgruntuvannyaPage() {
  let groups = [];
  let loadError = false;
  ...
  try {
    ...
    console.error('Failed to load obgruntuvannya-2 from uman.crl.net.ua:', err);
    loadError = true;
  }
      ...
      <section className="page-header-section relative pt-40 pb-16 lg:pt-48 lg:pb-20 bg-gray-950 overflow-hidden">
        ...
        <div className="relative z-10 max-w-screen-xl mx-auto px-6 lg:px-10">
          <nav className="flex items-center gap-2 text-xs text-white/40 mb-6" aria-label="Хлібні крихти">
            ...
            <span>/</span>
            <span className="text-white/70">Обґрунтування, відкриті торги</span>
          </nav>
          ...
          <h1 className="text-4xl lg:text-5xl font-300 text-white leading-tight">
            Обґрунтування <span className="font-600">закупівель</span>
          </h1>
      ...
      <section className="py-16 lg:py-20 bg-white">
        <div className="max-w-screen-xl mx-auto px-6 lg:px-10">
          <div className="reveal max-w-3xl mb-12">
            <p className="text-xs font-semibold tracking-widest text-blue-600 uppercase mb-4">Постанова КМУ від 11.10.2016 №710 (зі змінами)</p>
            <h2 className="text-2xl sm:text-3xl font-600 text-gray-900 leading-snug">
              Обґрунтування технічних і якісних характеристик та очікуваної вартості предмета закупівлі
            </h2>
          </div>
