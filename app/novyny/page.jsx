...
// new posts published on uman.crl.net.ua show up within that window without
// a redeploy, but repeat visits/filters are served instantly from cache
// instead of round-tripping to WP on every request.
...
const CATEGORY_FILTERS = [
  { key: 'all', label: 'Усі', ids: [CATEGORY_NOVYNY, CATEGORY_PORADY] },
  { key: 'novyny', label: 'Новини', ids: [CATEGORY_NOVYNY] },
  { key: 'porady', label: 'Поради лікаря', ids: [CATEGORY_PORADY] },
];
...
function buildHref({ category, sort, q, page }) {
...
export default async function NewsListPage({ searchParams }) {
  ...
  const categoryKey = CATEGORY_FILTERS.some((f) => f.key === sp.category) ? sp.category : 'all';
  const sort = sp.sort === 'oldest' ? 'oldest' : 'newest';
  const q = (sp.q || '').trim();
  const page = Math.max(1, parseInt(sp.page, 10) || 1);
  const activeFilter = CATEGORY_FILTERS.find((f) => f.key === categoryKey);
  ...
  try {
    ...
    console.error('Failed to load news from uman.crl.net.ua:', err);
    loadError = true;
  }
        ...
        <div className="max-w-screen-xl mx-auto px-6 lg:px-10">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 mb-6">
            <div className="flex flex-wrap items-center gap-2">
              {CATEGORY_FILTERS.map((f) => {
                ...
                return (
                  <Link
                    key={f.key}
                    href={buildHref({ category: f.key, sort, q })}
                    prefetch={false}
                    className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                      active ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-blue-300 hover:text-blue-700'
                    }`}
                  >
                    {f.label}
                  </Link>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <NewsSearchInput initialQuery={q} category={categoryKey} sort={sort} />
              ...
              <div className="flex items-center gap-2.5 bg-white border border-gray-200 rounded-full p-1 shadow-sm">
                ...
                <Link
                  href={buildHref({ category: categoryKey, sort: 'newest', q })}
                  prefetch={false}
                ...
                <Link
                  href={buildHref({ category: categoryKey, sort: 'oldest', q })}
                  prefetch={false}
          ...
          {q && (
            <p className="text-sm text-gray-500 mb-2">
              ...
              {' · '}
              <Link href={buildHref({ category: categoryKey, sort })} prefetch={false} className="text-blue-700 font-medium">Скинути</Link>
            </p>
          )}
        </div>
      </section>
        ...
        <div className="max-w-screen-xl mx-auto px-6 lg:px-10">
          {loadError ? (
            <div className="text-center py-20">
              <p className="text-gray-500 mb-4">Не вдалося завантажити новини — сайт лікарні тимчасово недоступний.</p>
              <Link href={buildHref({ category: categoryKey, sort, q, page })} prefetch={false} className="text-blue-700 font-medium text-sm">
                Спробувати ще раз
              </Link>
            </div>
          ) : posts.length === 0 ? (
          ...
          {totalPages > 1 && (
            <nav className="flex items-center justify-center gap-1.5 mt-14" aria-label="Пагінація">
              <Link
                href={buildHref({ category: categoryKey, sort, q, page: Math.max(1, page - 1) })}
                prefetch={false}
              ...
              {pages.map((p, i) => (
                <span key={p} className="flex items-center gap-1.5">
                  ...
                  <Link
                    href={buildHref({ category: categoryKey, sort, q, page: p })}
                    prefetch={false}
              ...
              <Link
                href={buildHref({ category: categoryKey, sort, q, page: Math.min(totalPages, page + 1) })}
                prefetch={false}
